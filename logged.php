        ...
        <header id="header">
            ...
            <div class="menu js-top-menu position-static hidden-sm-down" id="_desktop_top_menu">
                <ul class="top-menu container" id="top-menu" data-depth="0">
                    ...
                    <li class="category" id="category-403">
                        <a class="dropdown-item" href="update_profile.php" data-depth="0">
...
<?php
                $query = "SELECT * FROM `products` WHERE status = 'available' ORDER BY RAND() LIMIT 5 ";
                $result = mysqli_query($conn, $query) or die(mysqli_error($conn));
                if($result = mysqli_query($conn, $query)){
                    if(mysqli_num_rows($result)  > 0){
                        echo "
                        <div class='product-bottom' style='display: flex; flex-direction: row;'>
                        ";
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $image = $row['image'];
                            echo
                            "<div class='product-bottom' style='margin-bottom: 20px;'>";
                            if($row['status'] == 'S'){
                                echo "<p class='red'>20% off!!</p>";
                            }
                            echo
                                "<div class='i-con'>
                                    <img  style='postion: relative; width: 150px; margin-top: ; margin-right: 00px;' src='images/$image'>
                                </div>
                                <h4  style='margin: 40px; margin-top: 10px; font-size: 14px; '>Brand:$name</h4>
                                <div  style='margin: 40px; margin-top: -30px; font-size:10px;' class='stars'>
                                ...
                                if($row['status'] == 'S'){
                                    echo "<h5 class='price' style='margin-top: -50px;'><s>R$price</s></h5>";
                                    
                                }else{
                                  echo "<p style='margin: 50px; margin-top: -15px; font-size: 14px; font-weight: bold ' class='stars'>R$description</p>";
                                    echo "<p style='margin: 30px; margin-top: -15px; font-size: 15px; font-weight: bold; color: red; ' class='stars'>R$price.00</p>";
                                    echo "<p style='margin: 30px; margin-top: -15px; font-size: 15px; font-weight: bold ' class='stars'>Reviews(s):0</p>";
                                ...
                                <form action='load-page.php' method='post'>
                                    <input type='hidden' name='id' value='$id'>";
                                        ...
                                        if(isset($_SESSION["p$id"])){
                                            echo("
                                            <button name='remove_from_cart' class='btn' type='button'>Item in cart</button>
                                            ");
                                        }else {
                                            echo("
